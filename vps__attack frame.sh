...
#   
#   
#   -ne 0   ===   不等于     （shell的启动参数不等于1个）
#   -eq 0   ===   等于        (shell的启动参数等于1个)
#
#   sleep 2   睡眠2秒
...
if [ $? -eq 0 ] ;then
        apt-get install gcc gdb make gedit tmux python docker.io proxychains proxychains4 cmake socat telnet tree tcpdump iptraf iftop nethogs lrzsz git unzip p7zip-full curl wget vim openssl libssl-dev libssh2-1-dev -y >/dev/null 2>&1
        if [ $? -eq 0 ] ;then
            echo -e "\e[94m 常用基础工具及相关依赖库安装已完成 ! \e[0m"
            sleep 5 && cd
            echo -e "========================================================================================================="
        else
...
echo -e "\e[94m 开始安装子域名工具 请稍后... ! \e[0m" && sleep 2
...
if [ $? -eq 0 ] ;then
    ...
    if [ $? -eq 0 ] ;then
        echo -e "\e[37mOneForAll 已安装成功 ! \e[0m\n"
        git clone --recursive https://github.com/aboul3la/Sublist3r.git >/dev/null 2>&1
        if [ $? -eq 0 ] ;then
            echo -e "\e[36m Sublist3r 已经下载完成 ! \e[0m" && sleep 2 
            cd Sublist3r/ &&  pip2.7 install -r requirements.txt >/dev/null 2>&1 && python sublist3r.py -h >/dev/null 2>&1
            if [ $? -eq 0 ] ;then
                echo -e "\e[37m Sublist3r 已安装成功 ! \e[0m\n" && cd ../
                mkdir Sublist3r && cd Sublist3r
                ...
                if [ $? -eq 0 ] ;then
                    ...
                    if [ $? -eq 0 ] ;then
                        ...
                        if [ $? -eq 0 ] ;then
                            ...
                            if [ $? -eq 0 ] ;then
                                echo -e "\e[37m theHarvester 已安装成功 ! \e[0m\n" && cd ../
                                echo -e "========================================================================================================="
                            else
